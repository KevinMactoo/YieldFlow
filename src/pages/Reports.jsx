...
import { monthlyRevenue, expensesByCategory } from '../data/mockData'
...
export default function Reports() {
  ...
  const cropStatusData = Object.entries(cropsByStatus).map(([status, count]) => ({ status, count }))
  ...
  const productRevenue = salesData.reduce((acc, s) => {
    acc[s.product] = (acc[s.product] || 0) + Number(s.total)
    return acc
  }, {})
    ...
    <Layout title="Reports & Analytics">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: 'Crops Active',    value: crops.filter(c=>c.status!=='Harvesting').length, color: 'text-blue-700 bg-blue-50' },
        ].map(k => (
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-3 gap-2 text-center">
            ...
            <div>
              <p className="text-xl font-bold text-gray-900">{flocks.reduce((s,f)=>s+f.count,0).toLocaleString()}</p>
